...
(function(Buffer) {
  ...
  THREE.ColladaLoader.prototype = {
    ...
    options: {set convertUpAxis(value) {
    ...
    parse: function(text) {
      ...
      function buildNode(data) {
        ...
        for (var i = 0,
            l = instanceCameras.length; i < l; i++) {
          objects.push(getCamera(instanceCameras[i]).clone());
        }
        for (var i = 0,
            l = instanceLights.length; i < l; i++) {
          objects.push(getLight(instanceLights[i]).clone());
        }
        for (var i = 0,
            l = instanceGeometries.length; i < l; i++) {

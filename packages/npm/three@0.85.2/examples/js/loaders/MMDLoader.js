...
(function(Buffer) {
    ...
    var initAnimation = function() {
      ...
      var createTrack = function(node, type, times, values, interpolations) {
        if (times.length > 2) {
          ...
          var interpolateStride = (stride === 3) ? 12 : 4;
          var aheadIndex = 2;
          var index = 1;
  ...
  THREE.MMDLoader.prototype.createMesh = function(model, texturePath, onProgress, onError) {
    ...
    var initVartices = function() {
      for (var i = 0; i < model.metadata.vertexCount; i++) {
        ...
        for (var j = 0,
            jl = v.position.length; j < jl; j++) {
          buffer.vertices.push(v.position[j]);
        }
        for (var j = 0,
            jl = v.normal.length; j < jl; j++) {
          buffer.normals.push(v.normal[j]);
        }
        for (var j = 0,
            jl = v.uv.length; j < jl; j++) {
    ...
    var initMaterials = function() {
      ...
      for (var i = 0; i < materialParams.length; i++) {
        ...
        m.shininess = Math.max(p.shininess, 1e-4);
        if (model.metadata.format === 'pmd') {
          ...
          var uuid = loadTexture(toonFileName, {
            isToonTexture: true,
            defaultTexturePath: isDefaultToonTexture(toonFileName)
          });
          ...
          var uuid = loadTexture(toonFileName, {
            isToonTexture: true,
            defaultTexturePath: isDefaultToon
          });
          m.gradientMap = getTexture(uuid, textures);
        }
        materials.push(m);
  ...
  THREE.MMDHelper.prototype = {
    ...
    updateIKParametersDependingOnPhysicsEnabled: function(mesh, physicsEnabled) {
      ...
      var bones = mesh.geometry.bones;
      for (var j = 0,
          jl = iks.length; j < jl; j++) {
        var ik = iks[j];
        var links = ik.links;
        for (var k = 0,
            kl = links.length; k < kl; k++) {
          var link = links[k];

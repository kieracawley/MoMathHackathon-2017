...
describe('types.js', function() {
  ...
  it('can handle MACSTRING in Central European encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 9, encoding), 'Betűtípus');
    assert.equal(hex(encode.MACSTRING('Betűtípus', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Betűtípus', encoding)), data);
    assert.equal(encode.MACSTRING('Betűtípus 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Croatian encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 7, encoding), 'Štampač');
    assert.equal(hex(encode.MACSTRING('Štampač', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Štampač', encoding)), data);
    assert.equal(encode.MACSTRING('Štampač 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Cyrillic encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 9, encoding), 'Шрифт Foo');
    assert.equal(hex(encode.MACSTRING('Шрифт Foo', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Шрифт Foo', encoding)), data);
    assert.equal(encode.MACSTRING('Шрифт 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Gaelic encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 6, encoding), 'Dúiṫċí');
    assert.equal(hex(encode.MACSTRING('Dúiṫċí', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Dúiṫċí', encoding)), data);
  ...
  it('can handle MACSTRING in Greek encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 17, encoding), 'Γραμματοσειρά .Ö.');
    assert.equal(hex(encode.MACSTRING('Γραμματοσειρά .Ö.', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Γραμματοσειρά .Ö.', encoding)), data);
    assert.equal(encode.MACSTRING('Γραμματοσειρά 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Icelandic encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 19, encoding), 'Þórisdóttir þó aðég');
    assert.equal(hex(encode.MACSTRING('Þórisdóttir þó aðég', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Þórisdóttir þó aðég', encoding)), data);
    assert.equal(encode.MACSTRING('Þórisdóttir 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Inuit encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 9, encoding), 'ᐸᐃᑉᐹᒨᕆᔾᔪᑦ');
    assert.equal(hex(encode.MACSTRING('ᐸᐃᑉᐹᒨᕆᔾᔪᑦ', encoding)), data);
    assert.equal(hex(encode.MACSTRING('ᐸᐃᑉᐹᒨᕆᔾᔪᑦ', encoding)), data);
  ...
  it('can handle MACSTRING in Roman encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 9, encoding), 'Üetlibärg');
    assert.equal(hex(encode.MACSTRING('Üetlibärg', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Üetlibärg', encoding)), data);
    assert.equal(encode.MACSTRING('Üetlibärg 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Romanian encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 8, encoding), 'Tipărire');
    assert.equal(hex(encode.MACSTRING('Tipărire', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Tipărire', encoding)), data);
    assert.equal(encode.MACSTRING('Tipărire 字体', encoding), undefined);
  ...
  it('can handle MACSTRING in Turkish encoding', function() {
    ...
    assert.equal(decode.MACSTRING(unhex('DE AD BE EF ' + data), 4, 8, encoding), 'Basılmış');
    assert.equal(hex(encode.MACSTRING('Basılmış', encoding)), data);
    assert.equal(hex(encode.MACSTRING('Basılmış', encoding)), data);
    assert.equal(encode.MACSTRING('Basılmış 字体', encoding), undefined);
  ...
  it('can handle INDEX', function() {
    ...
    assert.equal(hex(encode.INDEX([foo, bar])), '00 02 01 01 06 07 68 65 6C 6C 6F A2');
  ...
  it('can handle deeply nested TABLEs', function() {
    var expected = '00 01 00 00 00 0A 00 20 00 3A ' + '00 01 44 46 4C 54 00 08 00 04 00 00 00 00 FF FF 00 02 00 00 00 01 ' + '00 02 6C 69 67 61 00 0E 73 6D 63 70 00 14 00 00 00 01 00 01 00 00 00 01 00 00';
    var table = {
      fields: [{
        name: 'version',
        type: 'FIXED',

...
describe('parse.js', function() {
  ...
  describe('parseRecordList', function() {
    it('can parse a list of records', function() {
      ...
      var p = new Parser(testutil.unhex(data), 0);
      assert.deepEqual(p.parseRecordList({
        a: Parser.byte,
        b: Parser.uShort
      }), [{
        a: 0x12,
    ...
    it('can parse a list of records of predefined length', function() {
      var data = '12 34 56 78 9A BC';
      var p = new Parser(testutil.unhex(data), 0);
      assert.deepEqual(p.parseRecordList(2, {
        a: Parser.byte,
        b: Parser.uShort
      }), [{
        a: 0x12,
  ...
  describe('parseListOfLists', function() {
    it('can parse a list of lists of 16-bit integers', function() {
      var data = '0003 0008 000E 0016' + '0002 1234 5678' + '0003 DEAD BEEF FADE' + '0001 9876';
      var p = new Parser(testutil.unhex(data), 0);
      assert.deepEqual(p.parseListOfLists(), [[0x1234, 0x5678], [0xdead, 0xbeef, 0xfade], [0x9876]]);
    });
    ...
    it('can parse a list of lists of records', function() {
      var data = '0002 0006 0012' + '0002 0006 0009 12 34 56 78 9A BC' + '0001 0004 DE F0 12';
      var p = new Parser(testutil.unhex(data), 0);
  ...
  describe('parseCoverage', function() {
    it('should parse a CoverageFormat1 table', function() {
      var data = '0004 1234' + '0001 0005 0038 003B 0041 0042 004A';
    ...
    it('should parse a CoverageFormat2 table', function() {
      var data = '0004 1234' + '0002 0001 004E 0057 0000';
      var p = new Parser(testutil.unhex(data), 4);
      assert.deepEqual(p.parseCoverage(), {
        format: 2,
        ranges: [{
  ...
  describe('parseClassDef', function() {
    it('should parse a ClassDefFormat1 table', function() {
      var data = '0001 0032 001A' + '0000 0001 0000 0001 0000 0001 0002 0001 0000 0002 0001 0001 0000' + '0000 0000 0002 0002 0000 0000 0001 0000 0000 0000 0000 0002 0001';
      var p = new Parser(testutil.unhex(data), 0);
      assert.deepEqual(p.parseClassDef(), {
        format: 1,
        startGlyph: 0x32,
        classes: [0, 1, 0, 1, 0, 1, 2, 1, 0, 2, 1, 1, 0, 0, 0, 2, 2, 0, 0, 1, 0, 0, 0, 0, 2, 1]
      });
  ...
  describe('parseScriptList', function() {
    it('should parse a ScriptList table', function() {
      var data = '0004 1234' + '0003 68616E69 0014 6B616E61 0018 6C61746E 001C' + '0000 0000 0000 0000' + '000A 0001 55524420 0016' + '0000 FFFF 0003 0000 0001 0002' + '0000 0003 0003 0000 0001 0002';
      var p = new Parser(testutil.unhex(data), 0);
      assert.deepEqual(p.parseScriptList(), [{
        tag: 'hani',
        script: {
          defaultLangSys: undefined,
          langSysRecords: []
        }
      }, {
        tag: 'kana',
  ...
  describe('parseFeatureList', function() {
    it('should parse a FeatureList table', function() {
      var data = '0004 0000' + '0003 6C696761 0014 6C696761 001A 6C696761 0022' + '0000 0001 0000   0000 0002 0000 0001   0000 0003 0000 0001 0002';
      var p = new Parser(testutil.unhex(data), 0);
      assert.deepEqual(p.parseFeatureList(), [{
        ...
        feature: {
          featureParams: 0,
          lookupListIndexes: [0, 1]
        }
      }, {
        tag: 'liga',
        feature: {
          featureParams: 0,
          lookupListIndexes: [0, 1, 2]
        }
      }]);
      assert.equal(p.relativeOffset, 2);
    });
  ...
  describe('parseLookupList', function() {
    it('should parse a LookupList table', function() {
      var data = '0004 0000' + '0003 0008 0010 0018' + '0004 000C 0001 0018' + '0004 000C 0001 0028' + '0004 000C 0001 0038' + '1234 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000' + '5678 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000' + '9ABC';
